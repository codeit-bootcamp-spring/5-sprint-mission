...
public interface UserApi {
  ...
  @ApiResponses({
      ...
      @ApiResponse(
          ...
          content = @Content(examples = @ExampleObject(value = "username/email already exists"))
      )
  })
  ResponseEntity<UserDto> create(
      @Parameter(
          description = "User 생성 정보(JSON 파트)",
          content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
              schema = @Schema(implementation = UserCreateRequest.class))
      ) UserCreateRequest userCreateRequest,
      @Parameter(
          description = "User 프로필 이미지(file 파트)",
  ...
  @ApiResponses({
      ...
      @ApiResponse(
          responseCode = "409",
          description = "같은 email 또는 username를 사용하는 User가 이미 존재함",
          content = @Content(examples = @ExampleObject("username/email already exists"))
      )
  })
  ResponseEntity<UserDto> update(
      @Parameter(description = "수정할 User ID") UUID userId,
      @Parameter(
          description = "수정할 User 정보(JSON 파트)",
          content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
              schema = @Schema(implementation = UserUpdateRequest.class))
      ) UserUpdateRequest userUpdateRequest,
      @Parameter(
          description = "수정할 User 프로필 이미지(file 파트)",
          content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE,
              schema = @Schema(type = "string", format = "binary"))
      ) MultipartFile profile
  );
