...
public class MessageController { // 컨트롤러 클래스 시작
    ...
    @PostMapping( // 생성은 POST 메서드로 명시
        path = "create", // 기존 라우트 이름 유지: /api/message/create
    ...
    @ApiResponses({ // Swagger: 가능한 응답 정의
        @ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = Message.class)) // 반환 스키마
        ),
        @ApiResponse(
            responseCode = "400",
    ...
    public ResponseEntity<Message> create( // 메시지 생성 엔드포인트
        ...
        Message createdMessage = messageService.create(messageCreateRequest,
            attachmentRequests); // 서비스 호출로 메시지 생성
    ...
    @PutMapping( // 수정은 PUT 메서드로 명시
        path = "update/{messageId}" // 기존 update 라우트 유지 + 식별자는 PathVariable로 적용
    )
    @Operation( // Swagger: 엔드포인트 요약/설명
        summary = "메시지 수정", // 요약
    ...
    @DeleteMapping( // 삭제는 DELETE 메서드로 명시
        path = "delete/{messageId}" // 기존 delete 라우트 유지 + 식별자는 PathVariable로 적용
    )
    @Operation( // Swagger: 엔드포인트 요약/설명
        summary = "메시지 삭제", // 요약
    ...
    @GetMapping( // 조회는 GET 메서드로 명시
        path = "findAllByChannelId/{channelId}" // 오타 수정: 슬래시 포함한 PathVariable 경로
    )
    @Operation( // Swagger: 엔드포인트 요약/설명
        summary = "채널별 메시지 목록 조회", // 요약
        description = "채널 식별자(UUID)로 해당 채널의 메시지 목록을 조회합니다." // 설명
        , operationId = "findAllMessagesByChannelId"
    )
    @ApiResponses({ // Swagger: 가능한 응답 정의
        @ApiResponse(
            responseCode = "200",
            description = "조회 성공",
            content = @Content(schema = @Schema(implementation = Message.class)) // 리스트 아이템 스키마
        ),
        @ApiResponse(
            responseCode = "404",
    ...
    public ResponseEntity<List<Message>> findAllByChannelId( // 채널별 메시지 목록 조회 엔드포인트
        @PathVariable("channelId") UUID channelId // 경로 변수에서 채널 ID 추출
    ) { // 메서드 시작
