...
public class UserController implements UserApi {
  ...
  public ResponseEntity<UserDto> create(
    ...
    log.info("Creating user {}", userCreateRequest.username());
    Optional<BinaryContentCreateRequest> profileRequest = Optional.ofNullable(profile)
        .flatMap(this::resolveProfileRequest);
    UserDto createdUser = userService.create(userCreateRequest, profileRequest);
    log.info("Created user name: {}, id: {} ", createdUser.username(), createdUser.id());
  ...
  public ResponseEntity<UserDto> update(
    ...
    log.info("Updating user {}", userUpdateRequest.newUsername());
    Optional<BinaryContentCreateRequest> profileRequest = Optional.ofNullable(profile)
        .flatMap(this::resolveProfileRequest);
    UserDto updatedUser = userService.update(userId, userUpdateRequest, profileRequest);
    log.info("Updated user {}", updatedUser.username());
    return ResponseEntity
  ...
  public ResponseEntity<Void> delete(@PathVariable("userId") UUID userId) {
    log.warn("Request to delete user with id={}", userId);
