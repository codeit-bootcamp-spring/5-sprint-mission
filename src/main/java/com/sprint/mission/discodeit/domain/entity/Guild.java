...
public class Guild extends AbstractEntity {
  ...
  public Guild(String name, boolean discoverable, UUID ownerId) {
    ...
    addUser(Objects.requireNonNull(ownerId, "ownerId must not be null."),
        Set.of(Permission.ADMINISTRATOR));
  ...
  public void setOwnerId(UUID userId) {
    ...
    if (isBanned(userId)) {
      throw new IllegalStateException("Banned user cannot be the owner.");
    }
    if (isNotMember(userId)) {
  ...
  public void addUser(UUID userId, Set<Permission> permissions) {
    Objects.requireNonNull(userId, "userId must not be null.");
    if (isBanned(userId)) {
      throw new IllegalStateException("User is banned from this guild.");
    }
  ...
  public void setPermissions(UUID userId, Set<Permission> newPermissions) {
    ...
    if (isBanned(userId)) {
      throw new IllegalStateException("Banned user cannot have permissions.");
    }
  ...
  public void addBan(UUID userId) {
    Objects.requireNonNull(userId, "userId must not be null.");
    if (isOwner(userId)) {
      throw new IllegalStateException("Cannot ban the guild owner.");
    }
    ...
    if (permissions.removeIf(p -> p.getUserId().equals(userId))) {
      changed = true;
    }
