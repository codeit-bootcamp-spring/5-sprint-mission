...
public abstract class AbstractEntity implements Serializable {
    ...
    public void restore() {
        if (!deleted && deletedAt == null && purgeAt == null) return;
        this.deleted = false;
        this.deletedAt = null;
        this.purgeAt = null;
        touch();
    ...
    public void schedulePurge(Instant purgeAt) {
        ...
        if (!deleted) throw new IllegalStateException("Call delete() first");
        if (purgeAt.isBefore(deletedAt)) throw new IllegalArgumentException("purgeAt must be >= deletedAt");
    ...
    private void applyDelete(Instant purgeAt) {
        ...
        if (!Objects.equals(this.purgeAt, purgeAt)) {
            ...
            this.purgeAt = purgeAt;
            mutated = true;
            if (purgeAt != null) onPurgeScheduled(purgeAt);
        }
        if (mutated) touch(current);
    }

    private void verifyInvariants() {
        if (updatedAt.isBefore(createdAt)) throw new IllegalStateException("updatedAt must be >= createdAt");
        if (!deleted) {
            if (deletedAt != null || purgeAt != null)
                throw new IllegalStateException("Non-deleted must have null deletedAt/purgeAt");
        } else {
            if (deletedAt == null) throw new IllegalStateException("Deleted must have non-null deletedAt");
        ...
        if (version < 0) throw new IllegalStateException("version must be >= 0");
    }
