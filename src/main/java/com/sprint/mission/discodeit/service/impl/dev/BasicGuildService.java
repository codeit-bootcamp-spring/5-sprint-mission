...
public class BasicGuildService implements DevGuildService {
    ...
    @Override
    public List<DevGuild> findGuildsJoinedByUser(UUID userId) {
    ...
    public void deleteGuild(UUID guildId, UUID ownerId) {
        ...
        for (UUID uid : new HashSet<>(guild.getUsers())) {
            DevUser user = userRepository.getOrThrow(uid);
    ...
    public void addChannel(UUID guildId, Channel channel) {
        ...
        channelRepository.getOrThrow(channel.getId());
        update(guildId, g -> g.addChannel(channel.getId()));
    }

    @Override
    public void removeChannel(UUID guildId, Channel channel) {
        Objects.requireNonNull(channel, "channel must not be null");
        update(guildId, g -> g.removeChannel(channel.getId()));
    }

    @Override
    public List<DevChannel> getChannels(UUID guildId) {
    ...
    @Override
    public Set<DevGuildPermissions> getMemberPermissions(UUID guildId, UUID userId) {
