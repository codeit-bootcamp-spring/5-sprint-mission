...
- [ ] `application.yaml`에 기본 로깅 레벨 설정 (`info`)
- [ ] 환경별 로깅 레벨 구성
    - 개발 환경: `debug`
    - 운영 환경: `info`
    - SQL 로그 레벨 유지
- [ ] Logback 설정 파일 추가 (`logback-spring.xml`)
    - 로그 패턴 정의
      ```
...
- [ ] 커스텀 예외 설계 (`com.sprint.mission.discodeit.exception[.{도메인}]`)
- [ ] `ErrorCode` Enum 정의 (예외 코드/메시지)
...
- [ ] 기존 예외 (`NoSuchElementException`, `IllegalArgumentException` 등) → 커스텀 예외로 대체
- [ ] 공통 응답 DTO: `ErrorResponse`
    - `int status`: HTTP 상태 코드
    - `String exceptionType`: 예외 클래스 이름
- [ ] 전역 예외 핸들러 (`@RestControllerAdvice`) 구현
    - 모든 예외를 `ErrorResponse` 형식으로 반환

### 유효성 검사

- [ ] Spring Validation 의존성 추가
- [ ] 주요 Request DTO 제약 조건 추가
    - `@NotNull`, `@NotBlank`, `@Size`, `@Email` 등
- [ ] 컨트롤러 메소드에서 `@Valid` 적용
- [ ] 검증 실패 예외 처리 (`MethodArgumentNotValidException`)
...
- [ ] `Mockito`로 Repository 모킹
- [ ] `BDDMockito` 활용해 가독성 개선
...
- [ ] **Discodeit 프로젝트에 Client 적용**
    ...
    - **설정 정보 추가**
        ...
        - `application-prod.yml`
          ...
          ```
- [ ] **Admin 대시보드에서 Discodeit 인스턴스 확인**
- [ ] **메트릭 확인**
    - 주요 API 요청 횟수, 응답 시간
    - 서비스 정보
